...
function get_query(node, query, watcher, context) {
	...
	for (var i = 0; i < props.length; i++) {
		...
		if (tag != "[object Object]" && tag != "[object Array]") {
			...
			var child = get_node_prop(node, prop, watcher, context);
			if (subQuery == 1) {
				result[prop] = get_all_node_props(child, watcher, context);
			} else if (get_tag(subQuery) == "[object Object]") {
				if (subQuery._) {
					result[prop] = handle_map_op(child, subQuery, watcher, context);
				} else {
					result[prop] = get_query(child, subQuery, watcher, context);
				}
			} else {
